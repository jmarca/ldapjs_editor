...
function createNewUser(req,next){
    ...
    binder(client,function(err){
        ...
        query(err,dsn,_userSearchAttributes(req),client,function(err,existing){
                           ...
                           async.waterfall([function(cb){
                                           ...
                                           ,function(user,barePassword,cb){
                                                ...
                                                addUserToGroup(ug_req
                                                              ,function(err,group){
                                                                   if(err) console.log('baka baka' + JSON.stringify(err))
...
function loadUsers(req,next){
    ...
    binder(client,function(err){
        ...
        query(err,dsn,_userSearchAttributes(req),client,function(err,users){
            client.unbind()
            // stupid camel case idiocy when case is not sensitive
...
function deleteUser(req,next){
    ...
    async.waterfall([function(cb){
                         ...
                         loadUser(req
                                 ,function(e,u){
                                      if(e) {
                                          console.log('choked on get'+JSON.stringify(e))
                                      }
                                      cb(e,u)
                                  })
                    ...
                    ,function(user,cb){
                         ...
                         console.log('groups membership is '+groups)
                         var funcs = [];
                         if(!_.isEmpty(groups)){
                             funcs = _.map(groups
                                          ,function(g){
...
function addUserToGroup(req,next){
    function _autg (err,group){
        ...
        _.forEach(req.param('uniquemember')
                 ,function(user){
                      if(!_.isObject(user)){
                          user = {uid:user}
                      }
...
function removeUserFromGroup(req,next){
    ...
    loadGroup(req
             ,function(err,group){
                  ...
                  var change = []
                  var drops = []
                  var dropmembers = req.param === undefined ? req.params.dropmembers
                                  :req.param('dropmembers')

                  group.uniquemember = _.flatten([group.uniquemember]) // handle non arrays
                  _.forEach(dropmembers
                           ,function(user){
                                if(!_.isObject(user)){
                                    user = {uid:user}
                                }
