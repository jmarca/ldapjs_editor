...
function _lowercase_attributes(entry){
    ...
    if(entry.uniquemember !== undefined){
        entry.uniquemember = _.flatten([savedAttrs.uniquemember])
    }
    return entry;
}
...
function loadGroup(req,res,next){
    ...
    binder(client,function(err){
        if(err) next(err);
        var dsn = 'cn='+req.params.cn+',ou=groups,dc=ctmlabs,dc=org';
        query(err,dsn,client,function(err,group){
            client.unbind()
            // stupid camel case idiocy when case is not sensitive
...
function loadUsers(req,res,next){
    ...
    binder(client,function(err){
        ...
        query(err,dsn,client,function(err,users){
            client.unbind()
            // stupid camel case idiocy when case is not sensitive
...
function loadGroups(req,res,next){
    ...
    binder(client,function(err){
        ...
        query(err,dsn,client,function(err,groups){
            client.unbind()
            next(err,groups)
        });
        return null;
...
function addUserToGroup(req,res,next){
    ...
    loadGroup(req
             ...
             ,function(err,group){
                  ...
                  _saveChange(group,change,function(err){
                      if(err) return next(err)
