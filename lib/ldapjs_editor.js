...
function handleNonPasswordFields(user,req,res,next){
    user.uid=req.params.uid;
    user.mail=req.params.mail
    user.givenName=req.params.givenName;
    user.sn=req.params.sn;
...
function handleNewPassword(user,req,res,next){
    ...
    ssha.checkssha(currentPassword
                  ,user.userPassword
                  ,function(err,result){
                       if(err || ! result){ return res.redirect(wrongPassword) }
                       ssha.ssha_pass(req.params.userPassword,function(err,hash){
                           if(err) return next(err);
                           user.userPassword = hash;
                           handleNonPasswordFields(req,res,user,next);
                           return null;
                       })
                       return null;
                   })
...
function editUser(req,res,next){
    loadUser(req,res,function(err,user){
        ...
        if(req.params.userPassword !== undefined){
            handleNewPassword(user,req,res,next);
        }else{
            handleNonPasswordFields(user,req,res,next);
        }
        return null;
    })
    return null;
