...
if(!group_dsn_postfix || !user_dsn_postfix || !manager_dn || !manager_password ){
    throw new Error("ldapjs_editor requires some environment variables to be set.  The following are required:\n"
                   +"  LDAP_GROUP_POSTFIX (for example 'ou=groups,dc=ctmlabs,dc=org')  "
                   +"  LDAP_USER_POSTFIX (for example 'ou=people,dc=ctmlabs,dc=org') "
                   +"  LDAP_DN (for example 'cn=Manager,dc=ctmlabs,dc=org')"
                   +"  LDAP_PASS (for example 'four score and seven years ago our')")
}
...
// slightly less braindead version of dsn_regex that accepts more general input
//
var dsn_regex = new RegExp(user_dsn_postfix+'|'+group_dsn_postfix)
...
// calls next with (error, user, barepassword)
...
function getDSN(user){
    if(_.isObject(user) ){
        if(user.dsn && dsn_regex.test(user.dsn)){
            ...
            return 'uid='+user.uid+',' + user_dsn_postfix
        }
    }else if(dsn_regex.test(user)){
        return user
    }
    return 'uid='+user+',' + user_dsn_postfix
}
...
function getGroupDSN(group){
    if(_.isObject(group) ){
        if(group.dsn && dsn_regex.test(group.dsn)){
            ...
            return 'cn='+group.cn+','+group_dsn_postfix;
        }
